    ...
    public partial class FrmTaskList : Form
    {
        ...
        private void FrmTaskList_Load(object sender, EventArgs e)
        {
            ...
            if (!UserStatic.isAdmin)
            {
                ...
                btnApprove.Location = new Point(214, 15);
                btnClose.Location = new Point(494, 15);
                pnlForAdmin.Hide();
        ...
        {
            if (detail.TaskID == 0)
                MessageBox.Show("Please select task on table");
            else
            {
                FrmTask frmTask = new FrmTask();
                frmTask.isUpdate = true;
                frmTask.detail = detail;
                this.Hide();
                frmTask.ShowDialog();
                this.Visible = true;
                FillAllData();
        ...
        private void btnApprove_Click(object sender, EventArgs e)
        {
            if (UserStatic.isAdmin && detail.taskStateID == TaskStates.OnEmployee && detail.EmployeeID != UserStatic.EmployeeID)
                MessageBox.Show("Before approve a task employee have to deliverty task");
        ...
        {
            DialogResult = MessageBox.Show("Area you sure to delete this task", "Warning", MessageBoxButtons.YesNo);
